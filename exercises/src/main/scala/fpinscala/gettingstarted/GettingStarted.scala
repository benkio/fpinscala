...
object MyModule {
  ...
  def fib(n: Int): Int = {
    def go(x : Int, y : Int, acc1 : Int, acc2 : Int) : Int = (x,y) match {
      case (_,0) => 0
      case (_,1) | (_, 2) => 1
      case (a, b) if (a <= 2) => go(3, b, 1, 1)
      case (a, b) if (a == b) => acc1 + acc2
      case (a, b) => go(a+1, b, acc2, acc1 + acc2)
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = as.init.zip(as.tail).map(t => gt(t._1, t._2)).reduce((b1, b2) => b1 && b2)
